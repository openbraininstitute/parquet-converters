srun -Aproj16 -pinteractive -n4 touch2parquetp \
    $DATADIR/cellular/circuit-1k/touches/binary/touchesData*

set +x
hash module 2> /dev/null || . /etc/profile.d/modules.sh
module load nix/hpc/spykfunc-py3
set -x
...
python $script \*.parquet $DATADIR/cellular/circuit-1k/touches/parquet/\*.parquet
